...
class MiniPlaces(data.Dataset):
    ...
    def _download_dataset(self, data_folder):
        ...
        data_file = os.path.join(data_folder, os.path.basename(data_urls["data"]))
        ...
        if (not require_download) and os.path.exists(
            os.path.join(data_folder, "miniplaces")
        ):
            # only download the annotations
            download_url(data_urls[self.split], os.path.join(data_folder, "miniplaces"))
        else:
    ...
    def _load_dataset(self, file_label_list):
        cached_filename = os.path.join(
            self.root_folder, "cached_{:s}.pkl".format(self.split)
        )
        if os.path.exists(cached_filename):
            ...
            try:
                ...
                print(
                    "Can't load cached file. Please remove the file and rebuild the cache!"
                )
        else:
            # load dataset into memory
            print(
                "Loading {:s} set into memory. This might take a while ...".format(
                    self.split
                )
            )
    ...
    def get_index_mapping(self):
        ...
        if not os.path.exists(train_label_file):
            raise ValueError("Label file {:s} does not exist!".format(train_label_file))
